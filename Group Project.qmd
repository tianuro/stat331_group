...
### 2.1.1 Visualize the relatioship between the quantitative variables
```{r}
data |>
  ggplot(aes(x = income,
             y = market_value)) +
...
### 2.1.2 Visualize the relatioship between the quantitative variables
```{r}
data |>
...
data_means <- data |>
  group_by(country) |> # grouping by yr
  summarize(
